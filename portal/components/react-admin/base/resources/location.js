import {
    ...
    SimpleForm,
    TextField,
    TextInput, useFilterState,
    useGetList,
    useGetMany, useInitializeFormWithRecord, useListContext, useListFilterContext,
    useNotify, useQuery,
    useRedirect,
} from "react-admin";
...
import {GET_DISTRICTS} from "@/components/react-admin/customHasura/records/districts";
...
export const LocationList = (props) => {
    ...
    const blocks = useMemo(() => {
        ...
        return _.uniqBy(districtData.filter((d) => d.district === selectedDistrict), 'block').map((a) => {
            return {
                id: a.block,
                name: a.block,
            }
    ...
    // }, []);
    const postFilters = [<TextInput label="Search" source="id" alwaysOn/>,
        <SelectInput label="District" onChange={(e) => setSelectedDistrict(e.target.value)} value={selectedDistrict}
                     source="district" choices={districts}/>,
        selectedDistrict ? <SelectInput label="Block" source="block" choices={blocks}/> : <></>,
...
export const LocationEdit = (props) => {
    ...
    const onSuccess = async ({data}) => {
        if (data) {
            notify("ra.notification.updated", "info", {smart_count: 1}, props.mutationMode === "undoable");
            const {delivery_status} = data;
            const [template, templateId, variables] = getTemplateFromDeliveryStatus(delivery_status);
            if (template && variables && session.role) {
                ...
                let replacedVariables = variables.map((keys) => //turn "ef" or "ab.cd" into ["ef"] and ["ab", "cd"] respectively
                ...
                if (response?.success) notify(response.success, "info"); else if (response?.error) notify(response.error, "warning");
